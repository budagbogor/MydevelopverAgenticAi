...
from config import TELEGRAM_BOT_TOKEN, ALLOWED_TELEGRAM_WHITELIST, update_env_project_root, PROJECT_ROOT, GITHUB_TOKEN, VERCEL_TOKEN
...
class TelegramBot:
    ...
    async def handle_new_project(self, update, context):
        ...
        new_path = os.path.join(r"I:\projectWebApps2026", project_name)
        try:
            if not os.path.exists(new_path): os.makedirs(new_path)
            if update_env_project_root(new_path):
                await update.message.reply_text(f"✅ Proyek diperbarui ke: {new_path}\n🚀 **Membuka Jendela Trae Baru...**")
                # Paksa restart IDE agar membuka folder baru
    ...
    async def handle_message(self, update, context):
        ...
        asyncio.create_task(self.orchestrator.process_task(update.message.text, update))

    def run(self):
        print("🤖 DarkSky Bot with SEARCH is running...")
